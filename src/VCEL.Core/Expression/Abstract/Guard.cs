...
{
    public Guard(
        IReadOnlyList<(IExpressionNode Condition, IExpressionNode Expression)> clauses, IExpressionNode otherwise)
    {
        Clauses = clauses;
        Otherwise = otherwise;
    }

    [JsonProperty("$type")] public NodeType Type => NodeType.Guard;
