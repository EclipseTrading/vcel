...
public class ExpressionNodeMapper<T>
{
    ...
    public IExpression<T> ToExpression(IExpressionNode? node) => node switch
    {
        Ternary n => factory.Ternary(
            ToExpression(n.Condition),
            ToExpression(n.TrueExpression),
            ToExpression(n.FalseExpression)),
        Let n => factory.Let(
            n.Bindings.Select(binding => (binding.Binding, ToExpression(binding.Expression))).ToArray(),
        ...
        null => throw new Exception($"Expression node not handled because it is null"),
        _ => throw new Exception($"Expression node type not handled '{node.Type}'"),
    };

    public IExpressionNode ToExpressionNode(IExpression<T>? node) => node switch
    {
        Ternary<T> e => new Ternary(
            ToExpressionNode(e.ConditionExpr),
            ToExpressionNode(e.TrueExpr),
            ToExpressionNode(e.FalseExpr)),
        LetExpr<T> e => new Let(
            e.Bindings.Select(binding => (binding.Item1, ToExpressionNode(binding.Item2))).ToArray(),
        ...
        BetweenExpr<T> e => new Between(
            ToExpressionNode(e.Left),
            ToExpressionNode(e.Lower),
            ToExpressionNode(e.Upper)),
        ...
        null => throw new Exception($"Expression not handled because it is null"),
        _ => throw new Exception($"Expression type not handled '{node.GetType()}'"),
    };
}